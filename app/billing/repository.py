...
class SqlBillingRepository:
    ...
    async def get_subscription(self, user_id: UUID) -> Subscription | None:
        ...
        if not r:
            return None
        return Subscription(
            id=r["id"], user_id=r["user_id"], plan=Plan(r["plan"]),
    ...
    async def upsert_subscription(self, sub: Subscription) -> None:
        await self.s.execute(text("""
            ...
            ON CONFLICT (id) DO UPDATE SET
              ...
              updated_at = now()
        """), {
            "id": str(sub.id), "uid": str(sub.user_id),
            "p": sub.plan.value, "st": sub.status.value,
            "prov": sub.provider.value, "psid": sub.provider_subscription_id,
            "cpe": sub.current_period_end, "cape": sub.cancel_at_period_end,
            "te": sub.trial_end,
        })
    ...
    async def list_invoices(
        ...
        rows = (await self.s.execute(text(f"""
