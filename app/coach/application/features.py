...
_breaker_feat = CircuitBreaker(name="openai_coach_features", fail_threshold=3, recovery_timeout_s=30)
...
async def foto_cross_check(
    session: AsyncSession, *, user_id: UUID, meal_time: str, detected_kcal: int,
...
async def macro_repair(session: AsyncSession, *, user_id: UUID) -> None:
    ...
    snacks = (await session.execute(text("""
