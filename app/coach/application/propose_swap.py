...
class ProposeSwap:
    ...
    async def __call__(
        self, *, user_id: UUID, plan_meal_id: UUID, top_k: int = 3,
    ) -> list[dict]:
        ...
        """), {"pmid": str(plan_meal_id), "uid": str(user_id)})).first()
        if not row:
        ...
        rows = (await self.session.execute(text("""
            ...
            SELECT r.id::text, r.name_en, r.kcal, r.protein_g,
             ...
             LIMIT :k
        """), {
            "rid": current_recipe_id, "mt": meal_time, "reg": region,
            "al": "{" + ",".join(allergies) + "}", "k": top_k,
        })).all()
        return [
            {"recipe_id": r[0], "name_en": r[1], "kcal": r[2],
             "protein_g": r[3], "distance": float(r[4])}
            for r in rows
        ]
