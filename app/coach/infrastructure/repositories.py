...
class SqlConversationRepository:
        ...
        if conv_id is not None:
            ...
            if m:
                return Conversation(
                    ...
                    locale=m.locale, created_at=m.created_at,
                )
        now = datetime.now(timezone.utc)
        new = ConversationModel(
            id=uuid4(), user_id=user_id, title=None, locale="en", created_at=now,
        )
        self.s.add(new)
        await self.s.flush()
    ...
    async def append_message(self, msg: Message) -> None:
        self.s.add(MessageModel(
            id=msg.id, conv_id=msg.conv_id, role=msg.role.value,
            content=msg.content, tokens_in=msg.tokens_in, tokens_out=msg.tokens_out,
    ...
    async def list_for_user(
        ...
        if len(rows) > limit:
            ...
            rows = rows[:limit]
        out = [
            Conversation(id=r.id, user_id=r.user_id, title=r.title,
                         locale=r.locale, created_at=r.created_at)
            for r in rows
        ]
    ...
    async def get_messages(
        ...
        out = [
            Message(
                id=r.id, conv_id=r.conv_id, role=Role(r.role),
                content=r.content, tokens_in=r.tokens_in, tokens_out=r.tokens_out,
