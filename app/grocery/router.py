...
def _to_out(gl) -> GroceryListOut:
    return GroceryListOut(
        id=gl.id, plan_id=gl.plan_id,
        items=[GroceryItemOut(
            id=i.id, category=i.category.value, name=i.name,
            amount=i.amount, purchased=i.purchased,
        ) for i in gl.items],
    )
...
async def get_grocery_list(
    plan_id: UUID, current_user: CurrentUserDep, session: SessionDep,
...
async def add_item(
    ...
    it = await uc(
        list_id=body.list_id, name=body.name, amount=body.amount,
...
async def shared_view(
    list_id: UUID, session: SessionDep, token: str = Query(...),
) -> GroceryListOut:
