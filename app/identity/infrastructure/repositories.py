...
def _user_from_model(m: UserModel) -> User:
    return User(
        ...
        oauth_provider=m.oauth_provider, oauth_subject=m.oauth_subject,
        email_verified=m.email_verified, role=m.role,
        created_at=m.created_at, last_login_at=m.last_login_at,
        deletion_requested_at=m.deletion_requested_at, deleted_at=m.deleted_at,
    )
...
def _token_from_model(m: RefreshTokenModel) -> RefreshToken:
    return RefreshToken(
        ...
        family_id=m.family_id, parent_id=m.parent_id,
        expires_at=m.expires_at, revoked_at=m.revoked_at,
        reused_at=m.reused_at, created_at=m.created_at,
    )


def _otp_from_model(m: OtpCodeModel) -> OtpCode:
    return OtpCode(
        id=m.id, user_id=m.user_id, code_hash=m.code_hash,
        purpose=m.purpose,  # type: ignore[arg-type]
        expires_at=m.expires_at, attempts=m.attempts,
        locked_until=m.locked_until, created_at=m.created_at,
    )
...
class SqlUserRepository:
    ...
    async def get_by_email(self, email: str) -> User | None:
        m = (await self.s.execute(select(UserModel).where(UserModel.email == email))).scalar_one_or_none()
        return _user_from_model(m) if m else None
    ...
    async def add(self, user: User) -> None:
        self.s.add(UserModel(
            ...
            oauth_provider=user.oauth_provider, oauth_subject=user.oauth_subject,
    ...
    async def update(self, user: User) -> None:
        await self.s.execute(update(UserModel).where(UserModel.id == user.id).values(
...
class SqlRefreshTokenRepository:
    ...
    async def add(self, token: RefreshToken) -> None:
        self.s.add(RefreshTokenModel(
            id=token.id, user_id=token.user_id, token_hash=token.token_hash,
            family_id=token.family_id, parent_id=token.parent_id,
            expires_at=token.expires_at, revoked_at=token.revoked_at,
            reused_at=token.reused_at, created_at=token.created_at,
        ))
        await self.s.flush()
    ...
    async def revoke(self, token_id: UUID, at: datetime) -> None:
        await self.s.execute(
            update(RefreshTokenModel).where(RefreshTokenModel.id == token_id)
            .values(revoked_at=at),
        )

    async def revoke_family(self, family_id: UUID, at: datetime) -> int:
        res = await self.s.execute(
            update(RefreshTokenModel).where(
...
class SqlOtpRepository:
    ...
    async def add(self, otp: OtpCode) -> None:
        self.s.add(OtpCodeModel(
            id=otp.id, user_id=otp.user_id, code_hash=otp.code_hash,
            purpose=otp.purpose, expires_at=otp.expires_at,
            attempts=otp.attempts, locked_until=otp.locked_until,
    ...
    async def get_active(self, user_id: UUID, purpose: OtpPurpose) -> OtpCode | None:
        ...
        ).order_by(OtpCodeModel.created_at.desc()).limit(1)
        m = (await self.s.execute(stmt)).scalar_one_or_none()
        return _otp_from_model(m) if m else None

    async def increment_attempts(self, otp_id: UUID) -> int:
        res = await self.s.execute(
            update(OtpCodeModel).where(OtpCodeModel.id == otp_id)
