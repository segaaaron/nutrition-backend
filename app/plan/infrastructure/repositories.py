...
def _meal_from_model(m: PlanMealModel) -> PlanMeal:
    return PlanMeal(
        id=m.id, plan_day_id=m.plan_day_id,
        ...
        completed=m.completed, swapped_from=m.swapped_from,
    )


def _day_from_model(d: PlanDayModel) -> PlanDay:
    return PlanDay(
        id=d.id, plan_id=d.plan_id, day_index=d.day_index, date=d.date,
...
def _plan_from_model(m: PlanModel) -> Plan:
    return Plan(
        ...
        total_days=m.total_days, current_day=m.current_day,
        status=m.status,  # type: ignore[arg-type]
        goal=m.goal, meals_per_day=m.meals_per_day,
        preferences=list(m.preferences or []),
        kcal_target=m.kcal_target, version=m.version,
...
class SqlPlanRepository:
    ...
    async def get(self, plan_id: UUID, *, hydrate: bool = True) -> Plan | None:
        ...
        if hydrate:
            stmt = stmt.options(
                selectinload(PlanModel.days).selectinload(PlanDayModel.meals)
            )
    ...
    async def save(self, plan: Plan) -> Plan:
        m = PlanModel(
            id=plan.id, user_id=plan.user_id, type=plan.type,
            total_days=plan.total_days, current_day=plan.current_day,
            status=plan.status, goal=plan.goal,
            meals_per_day=plan.meals_per_day, preferences=plan.preferences,
            kcal_target=plan.kcal_target, version=plan.version,
            created_at=plan.created_at,
        )
        for d in plan.days:
            dm = PlanDayModel(
                id=d.id, plan_id=plan.id, day_index=d.day_index,
                date=d.date, completed=d.completed,
            )
            for meal in d.meals:
                dm.meals.append(PlanMealModel(
                    id=meal.id, plan_day_id=d.id, meal_time=meal.meal_time,
                    recipe_id=meal.recipe_id, kcal=meal.kcal,
                    protein_g=meal.protein_g, carbs_g=meal.carbs_g, fat_g=meal.fat_g,
                    water_ml=meal.water_ml, water_pct=meal.water_pct,
                    completed=meal.completed, swapped_from=meal.swapped_from,
                ))
            m.days.append(dm)
        self.s.add(m)
        await self.s.flush()
    ...
    async def update_meta(self, plan: Plan) -> None:
        await self.s.execute(
            update(PlanModel).where(PlanModel.id == plan.id).values(
                status=plan.status, current_day=plan.current_day, version=plan.version,
            )
        )
