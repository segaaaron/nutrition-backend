...
def _row_to_entity(r) -> FastingSession:
    return FastingSession(
        id=r["id"], user_id=r["user_id"], method_h=r["method_h"],
        start_ts=r["start_ts"], end_ts=r["end_ts"],
        duration_s=r["duration_s"], target_s=r["target_s"],
        achieved=r["achieved"],
    )
...
class SqlFastingRepository:
    ...
    async def insert(self, fs: FastingSession) -> None:
        await self.s.execute(text("""
            ...
            VALUES (:id, :uid, :m, :s, :t)
        """), {
            "id": str(fs.id), "uid": str(fs.user_id), "m": fs.method_h,
            "s": fs.start_ts, "t": fs.target_s,
        })
    ...
    async def finalize(self, fs: FastingSession) -> None:
        await self.s.execute(text("""
            UPDATE fasting_sessions
             ...
             WHERE id = :id
        """), {
            "id": str(fs.id), "e": fs.end_ts,
            "d": fs.duration_s, "a": fs.achieved,
        })
