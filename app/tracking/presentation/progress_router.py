...
async def upload_progress_photo(
    ...
    """), {
        "id": str(pid), "uid": str(current_user),
        "w": weight_kg, "url": image_url,
    })
    return ProgressPhotoOut(
        id=pid, taken_at=datetime.utcnow(), weight_kg=weight_kg,
        image_url=image_url, blurhash=None,
    )
        ...
        "items": [
            {
                ...
                "image_url": r["image_url"], "blurhash": r["blurhash"],
            } for r in rows
        ],
        "next_cursor": nxt,
    }
...
async def delete_progress_photo(
    ...
    await session.execute(text("DELETE FROM progress_photos WHERE id = :id"),
                          {"id": str(photo_id)})
...
async def progress_dashboard(
    ...
    trend = (await session.execute(text("""
        SELECT day, weight_kg_mean
         ...
         ORDER BY day ASC
    """), {"uid": str(current_user)})).mappings().all()
    ...
    """), {"uid": str(current_user)})).mappings().first()
    # Goal progress
    ...
    return {
        "weight_trend": [
            {"day": r["day"].isoformat(), "weight_kg": float(r["weight_kg_mean"])}
            for r in trend
        ],
        "body_composition": dict(latest) if latest else None,
        "goal": dict(goal) if goal else None,
        "recent_photos": [
            {"id": str(p["id"]), "taken_at": p["taken_at"].isoformat(),
