...
**Thesis.** NOVA wins meal planning by being the only LatAm-first app where (a) every plan is **nutritionally safe by construction** — allergens and contraindications enforced by a closed-vocabulary DB enum (ADR-0001) before any LLM sees the candidate set — (b) every plan is **adaptive in 14 days** via the deterministic recalibration loop (ADR-0002) wired to plan regeneration, and (c) every coach answer is **grounded in the user's own plan + macro state**, not in generic chat. We treat OpenAI as the *coherence and conversation* layer, not the *retrieval* layer; that inversion is the cost moat ($0.40/active-user/day target, well below the $1.50 ADR-0004 cap).
...
**Where we accept being second**: *raw food-database breadth*. MyFitnessPal has ~14 M items; we will have ~10 k verified `foods` + 2 k `recipes` at MVP. We win by *quality* (verified, micronutrient-complete, LatAm-tagged) and offload long-tail logging to the vision+text NLP pipeline (spec §9.1). Investing to match MFP's count is a bad ROI and a nutrition-safety risk (FN allergen tags scale linearly with corpus size).
...
- **Test path**: `tests/integration/plan/test_coherence_call.py` (VCR'd), `tests/nutrition/test_plan_coherence_safety.py` (LLM cannot suggest a swap that violates allergens — strict post-validation).
...
- All suggestions written to `recipe_enrichment_proposals` table (new, additive; design only) with `status='pending'`. **Human nutritionist approves** in admin UI before write to `recipes`. No auto-merge — nutrition safety.
...
| **2** | **Snack inventory + Layers 1-2** | ≥ 100 snack recipes generated, validated, ingested; Layer-1 SQL filter + Layer-2 macro balancer behind `POST /plans` (no embeddings yet) | nutrition-generator + backend | `recipes_inventory_count{meal_time="snack"} ≥ 100`; `POST /plans` returns a macro-valid 3-meal plan; allergen-hard-exclude tests green | Sprint 1 |
...
| **4** | **Layer 4 LLM coherence + cache** | `plan_coherence_v1` prompt in `ai_prompts`; Arq task `generate_plan_coherence(plan_id)`; Redis cache layer; strict-JSON validation | backend + AI | < $0.02 per plan median in shadow mode; cache hit ≥ 30 % after 1 wk; `tests/nutrition/test_plan_coherence_safety.py` green | Sprint 3 |
...
|---|---|---|---|---|---|
| R1 | Allergen FN regression after a catalog re-ingest (a new batch reintroduces dairy-leaking recipes) | Ingest gate 5 lexicon expansion + CI block; deny-listed batches via `source_batch` | data-ops | `catalog_audit_gate_failed_total{gate="5"}` > 0 on a PR | `tests/data/test_catalog_nutrition_safety.py::test_no_known_allergen_in_ingredients_without_tag` |
...
| R5 | Snack inventory drought (cap not raised even after generation) | `recipes_inventory_count{meal_time="snack"}` gauge + alarm on `feature_flags.plans.snack_slot.enabled=true` mismatch | nutrition | gauge < 100 for > 14 d post-launch | `tests/data/test_catalog_coverage.py::test_every_meal_time_has_minimum_records` |
| R6 | Micronutrient backfill stalls ⇒ embarazo / anemia users get 422 indefinitely | Public dashboard of % coverage per nutrient; 4-week SLA owned by data-ops | data-ops + nutrition | coverage delta < 5 pp/week | `tests/data/test_micronutrient_coverage.py` (new) |
